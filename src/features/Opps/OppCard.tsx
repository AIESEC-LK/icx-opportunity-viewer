...
export default function OppCard({
	...
	const { containerP, textContainerP, textM, textW, textSize } = useMemo(() => responsiveResizes(isMobile), [isMobile]);
	console.log(dateOpened);
	return (
		<Container
			className={` ${containerP} opp-card rounded `}
			onClick={() => window.open(`https://aiesec.org/opportunity/${ProgramTypes.keys[program].link}/${id}`, "_blank")}
		>
			...
			<Media className="flex-wrap">
				...
				<Media.Body className={`${textContainerP} `}>
					...
					<p className={`${textM}`}>
						...
						{salaryInfo?.salary ? (
							<span>
								, {salaryInfo?.salary} {salaryInfo?.salary_currency?.alphabetic_code} {salaryInfo?.salary_periodicity}
							</span>
						) : null}
						{!sdg && salaryInfo && !salaryInfo.salary ? <span>, Unpaid</span> : null}
					</p>
					<p className={`${textM} ${textW} ${textSize} text-muted opp-card__description `}>{description}</p>
					{/* <div className="d-flex justify-content-between"> */}
						...
						{availableSlots.map((val: availableDateType, index: number) => (
							...
							>
								<Badge
									...
									key={`badge ${index}`}
									variant="light"
									bsPrefix={"badge bg-transparent "}
								>
									<p
										className="p-1 m-0 f12 font-weight-normal "
										key={`text ${index}`}
									>
										{format(parse(val.start_date as string, "yyyy-L-d", new Date()), "d MMM yy")} -{" "}
										{format(parse(val.end_date as string, "yyyy-L-d", new Date()), "d MMM yy")}
									</p>
								</Badge>
							</div>
						))}
